    ...
    >
      <div className="container-rubina flex h-16 items-center justify-between gap-4">
        ...
        <div className="flex shrink-0 items-center gap-1 sm:gap-2">
          {(["instagram", "whatsapp"] as const).map((id) => {
            const href =
              id === "instagram" ? site.contact.instagram : siteWhatsAppHref();
            return (
              <a
                key={id}
                href={href}
                ...
                aria-label={id === "instagram" ? "Instagram" : "WhatsApp"}
              >
                {id === "instagram" ? <InstagramIcon /> : <WhatsAppIconSm />}
              </a>
            );
          })}
        </div>
