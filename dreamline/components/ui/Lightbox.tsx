...
export function Lightbox({ src, alt, open, onClose }: LightboxProps) {
  ...
  return createPortal(
    <div
      className="fixed inset-0 z-[220] flex items-center justify-center bg-ink/90 p-4"
      role="dialog"
      aria-modal="true"
      aria-label={alt}
      onClick={onClose}
    >
      ...
      <div className="relative max-h-[85vh] w-full max-w-4xl" onClick={(e) => e.stopPropagation()}>
        <div className="relative aspect-[4/5] w-full sm:aspect-[4/3] lg:aspect-[16/10]">
          <Image src={src} alt={alt} fill unoptimized className="object-contain" sizes="100vw" priority />
        </div>
      </div>
    </div>,
    document.body,
