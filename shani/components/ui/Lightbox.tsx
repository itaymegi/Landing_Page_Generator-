...
export function Lightbox({
  ...
  return (
    <AnimatePresence>
      {open && current ? (
        <motion.div
          className="fixed inset-0 z-[110] flex items-center justify-center bg-brown/92 p-4"
          role="dialog"
          aria-modal="true"
        ...
        >
          <button
            type="button"
            onClick={onClose}
            className="absolute end-4 top-4 z-10 flex min-h-11 min-w-11 items-center justify-center rounded-full bg-white/10 text-white transition-colors hover:bg-white/20 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent"
            aria-label="סגור"
          >
            <svg
              className="h-6 w-6"
