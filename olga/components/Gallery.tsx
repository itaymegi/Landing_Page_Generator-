...
const previewImages = getGalleryPreviewImages();
    ...
    <AnimatePresence>
      {open ? (
        ...
        >
          ...
          <div className="container-olga py-8 sm:py-12">
...
export function Gallery() {
  const { gallery } = site;
  const [activeFilter, setActiveFilter] = useState<GalleryCategory>(
    DEFAULT_GALLERY_FILTER,
  );
  ...
  const [activeImages, setActiveImages] =
    useState<GalleryImage[]>(previewImages);
  ...
  const filteredPreview = useMemo(
    () => filterGalleryByCategory(previewImages, activeFilter),
    [activeFilter],
  );

  const filteredAll = useMemo(
    () => filterGalleryByCategory(allImages, activeFilter),
    [activeFilter],
  );
              ...
              <button
                ...
                className={`min-h-10 rounded-full border px-4 text-sm font-medium transition-all sm:px-5 ${
                  activeFilter === filter.id
                    ? "border-accent-deep bg-accent-deep text-white shadow-[0_4px_16px_rgba(168,137,106,0.3)]"
        ...
        <Reveal delay={200}>
          <div className="mt-10 flex justify-center sm:mt-12">
            <button
              ...
              className="inline-flex min-h-12 items-center justify-center rounded-full border border-accent/60 px-8 text-base font-medium text-accent-deep transition-all hover:border-accent hover:bg-accent/8"
            >
