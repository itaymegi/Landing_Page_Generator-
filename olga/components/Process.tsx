...
const ACCENT = "#c4a882";
const ACCENT_DEEP = "#a8896a";
const BLUSH = "#f8ece8";
const SAGE = "#8a9a85";
const TEXT = "#2a2826";
const MUTED = "#7a736c";

const STEP_ICONS = ["handshake", "pencil", "flower", "sparkle"] as const;
  ...
  return (
    <defs>
      <linearGradient id={`${id}-grad`} x1="0%" y1="0%" x2="100%" y2="100%">
        ...
        <stop offset="45%" stopColor="#EDE4D3" />
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{
          background:
            "radial-gradient(ellipse 70% 55% at 50% 48%, rgba(196,168,130,0.1) 0%, rgba(138,154,133,0.05) 40%, transparent 72%)",
        }}
      />
    </div>
  );
...
function ProcessCircle({
  ...
  return (
    <div
      className={`relative flex ${size} shrink-0 items-center justify-center rounded-full border-[1.5px] bg-white transition-all duration-700 ease-out lg:group-hover:-translate-y-0.5 lg:group-hover:shadow-[0_8px_28px_rgba(196,168,130,0.22)]`}
      style={{
        borderColor: isHighlighted ? ACCENT : `${ACCENT}BB`,
        backgroundColor: active ? ACCENT : completed ? BLUSH : "#FFFFFF",
        boxShadow: active
          ? "0 6px 24px rgba(196,168,130,0.35), 0 0 0 6px rgba(248,236,232,0.35)"
          : completed
            ? "0 4px 16px rgba(196,168,130,0.15)"
...
function ProcessMobileJourney({
  ...
  const gradId = "olga-mobile-stem";
  const revealSteps = !animate || activeIndex < 0;

  return (
    <div className="relative mx-auto mt-7 h-[min(60dvh,26rem)] w-full max-w-sm lg:hidden">
      <svg
...
function ProcessDesktopJourney({
  ...
  const gradId = "olga-desktop-stem";
...
export function Process() {
  ...
  return (
    ...
    >
      ...
      <div className="container-olga relative z-[1] mx-auto max-w-6xl">
