  ...
  return (
    <header
      className={`fixed inset-x-0 top-0 z-50 transition-all duration-300 ${
        scrolled
          ? "border-b border-border bg-parchment/95 shadow-sm shadow-brown/5 backdrop-blur-md"
          : "border-b border-transparent bg-transparent"
      }`}
    >
      <div className="container-munch flex h-16 items-center justify-between gap-4">
        <a href="#" className="group shrink-0" aria-label={`${site.brand.name} — דף הבית`}>
          <BrandLogo size="md" inverse={!scrolled} />
        </a>
        ...
        <div className="flex shrink-0 items-center gap-2 sm:gap-3">
          <a
            ...
            className={`inline-flex min-h-11 min-w-11 items-center justify-center rounded-full transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-gold ${
              scrolled ? "text-brown/70 hover:text-gold-deep" : "text-white/90 hover:text-white"
            }`}
